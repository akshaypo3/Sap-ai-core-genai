...
export const ChatInterface: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const SYSTEM_PROMPT = t("SYSTEM_PROMPT")
  ...
  return (
    <Card className="fixed bottom-20 right-4 w-1/3 h-2/5 flex flex-col">
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <div className="flex">
          <Button variant="ghost" size="icon" onClick={clearChat} title={t("Clear Chat")}>
            <Trash2 className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="icon" onClick={closeChat} title={t("Close Chat")}>
            <X className="h-4 w-4" />
          </Button>
        </div>
      </CardHeader>
