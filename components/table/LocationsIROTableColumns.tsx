...
export type LocationIRO = {
  ...
  export const columns_location_IRO: ColumnDef<LocationIRO>[] = [
    {
      accessorKey: "name",
      header: "Name",
    ...
    
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex justify-center gap-2"> {/* Adjust the gap as needed */}
          <Link href={`/materiality/company/${row.original.company_id}/location/${row.original.location_id}/${row.original.id}`}>
          ...
          <div>
            <DeleteLocationIROButton id={row.original} />
          </div>
        </div>
        );
      },
