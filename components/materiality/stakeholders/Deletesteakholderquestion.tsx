...
import { deleteStakeholderQuestions } from '@/lib/stakeholders/action';

export function DeleteStackholderQuestionButton({id}) {
  const [isOpen, setIsOpen] = useState(false);
  const [isPending, startTransition] = useTransition();

  const handleDelete = () => {
    startTransition(async () => {
      await deleteStakeholderQuestions(id.id,id.assessmentId);
      setIsOpen(false);
    });
  };
